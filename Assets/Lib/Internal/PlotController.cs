...
public class PlotController : MonoBehaviour {
	...
	public void updatePlot(List<SpectralFluxInfo> pointInfo, int curIndex = -1, float time = 0f) {
        ...
        List<SpectralFluxInfo> peaks = pointInfo.FindAll(point => point.isPeak);
        peakPoints.Clear();
        foreach(SpectralFluxInfo peak in peaks)
        {
            if (peak.time - previousTime > 1f)
            {
                peakPoints.Add(peak.time);
                print(peak.time);
