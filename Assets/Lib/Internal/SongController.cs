...
public class SongController : MonoBehaviour {
    ...
    public float MinDiff = 1f;
    public List<float> PeakTimes;
    public CreateNote Notes;
	...
	void Update() {
		...
		if (preProcessSamples) {
            ...
            PeakTimes.Clear();
            List<SpectralFluxInfo> Info = preProcessedSpectralFluxAnalyzer.spectralFluxSamples;
            List<SpectralFluxInfo> peaks = Info.FindAll(point => point.isPeak);
            PeakTimes.Clear();
            float previousTime = 0;
            foreach (SpectralFluxInfo peak in peaks)
            {
                if (peak.time - previousTime >= MinDiff)
                {
                    PeakTimes.Add(peak.time);
                    //print(peak.time);
                    previousTime = peak.time;
                }
            }
            Notes.EndTimes = PeakTimes;
            //preProcessedPlotController.updatePlot (preProcessedSpectralFluxAnalyzer.spectralFluxSamples, indexToPlot, audioSource.time);
        }
	}
